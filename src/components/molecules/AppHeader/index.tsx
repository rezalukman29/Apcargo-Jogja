...
const AppHeader: React.FC<IProps> = (props) => {
  ...
  useEffect(() => {
    if (document.location) {
      ...
      if (paramsId) {
        getDataJobsheetID(paramsId);
      }
    }
  }, []);
  const getDataJobsheetID = async (id: string) => {
  ...
  const aktifBuyHeader = useMemo(() => {
    return data && data?.aktif?.buy?.length
      ? Object.keys(data?.aktif?.buy[0])
      : null;
  }, [data]);

  const aktifBuyValue = useMemo(() => {
    return data ? data?.aktif?.buy?.map((el: any) => Object.values(el)) : null;
  }, [data]);

  const aktifSellHeader = useMemo(() => {
    return data && data?.aktif?.sell?.length
      ? Object.keys(data?.aktif?.sell[0])
      : null;
  }, [data]);

  const aktifSellValue = useMemo(() => {
    return data ? data?.aktif?.sell?.map((el: any) => Object.values(el)) : null;
  }, [data]);

  const pasifCostMasterHeader = useMemo(() => {
    return data && data?.pasif?.costmaster?.length ? Object.keys(data?.pasif?.costmaster[0]) : null;
  }, [data]);

  const pasifCostMasterValue = useMemo(() => {
    return data
      ? data?.pasif?.costmaster?.map((el: any) => Object.values(el))
      : null;
  }, [data]);

  const pasifFieldcostHeader = useMemo(() => {
    return data && data?.pasif?.fieldcost?.length ? Object.keys(data?.pasif?.fieldcost[0]) : null;
  }, [data]);

  const pasifFieldcostValue = useMemo(() => {
    return data
      ? data?.pasif?.fieldcost?.map((el: any) => Object.values(el))
      : null;
  }, [data]);

  const pasifHumicoHeader = useMemo(() => {
    return data && data?.pasif?.humico?.length ?  Object.keys(data?.pasif?.humico[0]) : null;
  }, [data]);

  const pasifHumicoValue = useMemo(() => {
    return data
      ? data?.pasif?.humico?.map((el: any) => Object.values(el))
      : null;
  }, [data]);
  ...
  return (
    <Box w="full" mb="25px">
        ...
        >
          ...
          <Flex alignItems="center" gridGap="24px" marginLeft="auto">
            {idParams && (
              ...
              >
                <button style={{ color: "white", fontWeight: 'bold', fontSize: 14, padding: 8, paddingLeft: 16, paddingRight: 16, backgroundColor: 'black', borderRadius: 8 }}> Export excel </button>
              </DownloadTableExcel>
            )}
            <Button
              color="#FFFFFF"
      ...
      <table ref={tableRef} style={{ color: "black", display: 'none' }}>
      <tbody>
          <tr>
            <th>Aktif - Buy</th>
          </tr>
        ...
        <tbody>
          <tr>
            {aktifBuyHeader &&
              aktifBuyHeader.map((a: string) => {
                return <th key={a}>{a}</th>;
              })}
          </tr>
          {aktifBuyValue &&
            aktifBuyValue.map((a: any[]) => {
              return (
                <tr>
                  {a.map((b: string | null, idx: number) => {
                    return (
                      <td key={`${idx}_aktifSell`}>{b === null ? "" : b}</td>
                    );
                  })}
                </tr>
              );
        ...
        <tbody>
          <tr>
            {aktifSellHeader &&
              aktifSellHeader.map((a: string) => {
                return <th key={a}>{a}</th>;
              })}
          </tr>
          {aktifSellValue &&
            aktifSellValue.map((a: any[]) => {
              return (
                <tr>
                  {a.map((b: string | null, idx: number) => {
                    return (
        ...
        <tbody>
          <tr>
            <th>Pasif - Costmaster</th>
          </tr>
        </tbody>
        <tbody>
          <tr>
            {pasifCostMasterHeader &&
              pasifCostMasterHeader.map((a: string) => {
                return <th key={a}>{a}</th>;
              })}
          </tr>
          {pasifCostMasterValue &&
            pasifCostMasterValue.map((a: any[]) => {
              return (
                <tr>
                  {a.map((b: string | null, idx: number) => {
                    return (
        ...
        <tbody>
          <tr>
            <th>Pasif - Fieldcost</th>
          </tr>
        </tbody>
        <tbody>
          <tr>
            {pasifFieldcostHeader &&
              pasifFieldcostHeader.map((a: string) => {
                return <th key={a}>{a}</th>;
              })}
          </tr>
          {pasifFieldcostValue &&
            pasifFieldcostValue.map((a: any[]) => {
              return (
                <tr>
                  {a.map((b: string | null, idx: number) => {
                    return (
        ...
        <tbody>
          <tr>
            {pasifHumicoHeader &&
              pasifHumicoHeader.map((a: string) => {
                return <th key={a}>{a}</th>;
              })}
          </tr>
          {pasifHumicoValue &&
            pasifHumicoValue.map((a: any[]) => {
              return (
                <tr>
                  {a.map((b: string | null, idx: number) => {
                    return (
